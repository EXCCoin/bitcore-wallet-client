...
function _checkNetwork(network) {
  if (!_.contains(['exccdlivenet', 'exccdtestnet'], network)) throw new Error('Invalid network');
};
...
Credentials._getNetworkFromExtendedKey = function(xKey) {
  ...
  return xKey.charAt(0) == 't' ? 'exccdtestnet' : 'exccdlivenet';
};
...
Credentials.prototype.getBaseAddressDerivationPath = function() {
  ...
  var coin = (this.network == 'exccdlivenet' ? "20" : "11");
