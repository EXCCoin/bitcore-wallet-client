...
var BASE_URL = 'https://bws.excc.co/bws/api';
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testMessageSigning(xpriv, xpub) {
    ...
    var priv = xpriv.derive(0, false).derive(0, false).privateKey;
    var signature = Utils.signMessage(message, priv);
    var pub = xpub.derive(0, false).derive(0, false).publicKey;
    return Utils.verifyMessage(message, signature, pub);
  };

  function testHardcodedKeys() {
    var words = "arrest suggest ice practice almost affair invite siege general universe utility runway";
    var xpriv = Mnemonic(words).toHDPrivateKey();

    if (xpriv.toString() != 'tprvZUo1ZuEfLLFWegkA5BVs3gs1T6TModh97DQxZPrC7r7BdytEXaaVbyyLh6bSnkkqf4ZvdJCHABodzJxBs8JF9vTDDekwjmeGTkww6CbyeZ9') return false;

    xpriv = xpriv.derive("m/44'/0'/0'");
    if (xpriv.toString() != 'tprvZcA91ytv3MomqztDhdnQXdMpMH7AVbaWjJCbtDiMQjMKhakh1D8KrkfHaDNnvvxZh9pVfwPqwD3fxVRSEFCSPHsK64Ha1bq2X4CPBF6dJEL') return false;

    var xpub = Bitcore.HDPublicKey.fromString('tpubVjW5r8sbu9p7JtnWoEoEFAuAQCkYbnZT2AT7dYcHZ1CAyjaXzXhRJ5yCHT4oV2dJthqm7BeyBu8feMszy1gUQAQiCjieQhYtqn4PThWNbKR');
  ...
  function testLiveKeys() {
    ...
    if (words && (!c.mnemonicHasPassphrase || opts.passphrase)) {
      ...
      xpriv = m.toHDPrivateKey(opts.passphrase, c.network).derive("m/44'/0'/0'");
    }
    if (!xpriv) {
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, cb) {
  ...
  self.getUtxos({
    ...
    return cb(null, _.sum(utxos, 'exels'));
  });
};
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var amount = _.sum(utxos, 'exels') - fee;
