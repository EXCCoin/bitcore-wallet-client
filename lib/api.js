...
var BASE_URL = 'http://bws.excc.co/bws/api';
...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.network || 'exccdlivenet');
};
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    var words = "Belfast amusement cobra provincial steamship Jupiter eyetooth recipe talon backwater repay belowground breadline megaton quota recover tiger responsive skydive unicorn scenic surrender spindle revival rematch belowground chopper Galveston python article Oakland Medusa python";
    var xpriv = Mnemonic(words).toHDPrivateKey();

    if (xpriv.toString() != 'dprv3mpAfpH1eQEMjPYcVaTwMrAPqhBXn9vgUaa3KoBjFPnGgXv5VrUMtSMYeqD1rurmQ5gRC4mnf9TzAKJEt8vgkVVc7W6Bn7xvTu6VQSg8CVg') return false;

    xpriv = xpriv.derive("m/44'/11/0'");
    if (xpriv.toString() != 'dprv3pUby3TJz3T1vfDjWQJY8eRv9YbYQd3Sis4HCXGyLVT6PxyCWoqAuzsHZTUxu1XWwd7TV9XxXWeMWjcf6JD1bbbu5NhrvvEnoob8QfUrF8n') return false;

    var xpub = Bitcore.HDPublicKey.fromString('dpubZGGhLB1c4RWoq57VkvMtesEAmZ9oqZdWvQgbwSDLLa3j7WzqPmAmzaPJiB575kcNY7TWbNYyix22w9215Wu9kc11L9TmHuGNq2ZHT5k6N7h');
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.network || 'exccdlivenet', opts.passphrase, opts.language || 'en', opts.account || 0);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.network || 'exccdlivenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44);
};
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, cb) {
  ...
  self.getUtxos({
    ...
    return cb(null, _.sum(utxos, 'atoms'));
  });
};
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var amount = _.sum(utxos, 'atoms') - fee;
...
API._buildSecret = function(walletId, walletPrivKey, network) {
  ...
  return _.padRight(widBase58, 22, '0') + walletPrivKey.toWIF() + (network == 'exccdtestnet' ? 'T' : 'L');
};
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    return {
      ...
      network: networkChar == 'T' ? 'exccdtestnet' : 'exccdlivenet',
    };
  } catch (ex) {
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || _.contains(['exccdlivenet', 'exccdtestnet'], network));

  self._doGetRequest('/v1/feelevels/?network=' + (network || 'exccdlivenet'), function(err, result) {
    if (err) return cb(err);
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  if (!_.contains(['exccdtestnet', 'exccdlivenet'], network)) return cb(new Error('Invalid network'));
